...
// ==============================
// INICIALIZAÃ‡ÃƒO - Verificar/Criar Colunas
// ==============================
(async () => {
  try {
    ...
    // Adicionar coluna 'removido' na tabela 'item' se nÃ£o existir
    try {
      await client.query(`
        ALTER TABLE item 
        ADD COLUMN IF NOT EXISTS removido BOOLEAN DEFAULT FALSE;
      `);
      console.log('âœ… Coluna "removido" verificada/criada na tabela "item"');
    } catch (altErr) {
      if (altErr.message.includes('already exists')) {
        console.log('â„¹ï¸ Coluna "removido" jÃ¡ existe na tabela "item"');
      } else {
        console.warn('âš ï¸ Aviso ao criar coluna removido:', altErr.message);
      }
    }

    client.release();
  } catch (err) {
    console.error('âŒ Erro ao conectar ao banco:', err.message || err);
  }
})();

const path = require('path');

// Adiciona isso antes das outras rotas
app.get('/', (req, res) => {
...
app.delete('/api/item/:id', async (req, res) => {
  ...
  console.log('ðŸ—‘ï¸ DELETE /api/item/:id recebido com id:', id);
  ...
  try {
    ...
    if (itemResult.rows.length === 0) {
      console.warn('Item nÃ£o encontrado:', id);
      return res.status(404).json({ error: 'Item nÃ£o encontrado' });
    }
    ...
    console.log('âœ… Item marcado como removido:', itemResult.rows[0].nome);
    res.json({ message: 'Item removido do estoque' });

  } catch (err) {
    console.error('âŒ Erro ao remover item:', err.message);
...
app.get('/api/relatorio/itens-mes', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar itens do mÃªs' });
  }
});
...
app.get('/api/relatorio/saidas', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar saÃ­das' });
  }
});
...
app.get('/api/relatorio/exportar/:tipo', async (req, res) => {
  ...
  try {
    ...
    doc.fontSize(18).text(`RelatÃ³rio - ${titulo}`, { align: 'center' });
    doc.moveDown();

    result.rows.forEach(row => {
      doc.fontSize(12)
        .text(`CÃ³digo: ${row.codigo}`)
        .text(`Nome: ${row.nome}`)
        .text(`DescriÃ§Ã£o: ${row.descricao}`)
        .text(`CondiÃ§Ã£o: ${row.condicao}`)
        .moveDown();
    });
...
app.post('/api/lucro', async (req, res) => {
  ...
  try {
    // Upsert: salva ou atualiza se jÃ¡ existir para o mesmo item
    const result = await pool.query(`
...
app.delete('/api/lucro/:id', async (req, res) => {
  try {
    ...
    res.json({ message: 'Lucro excluÃ­do' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
// Em Vercel e em ambientes serverless, NÃƒO chamamos app.listen â€” Vercel
// trata o export padrÃ£o como handler.
if (process.env.NODE_ENV === 'test') {
  // Para testes, manter um servidor escutando (pode ser necessÃ¡rio para alguns harnesses)
  server = app.listen();
} else if (!isVercel) {
  ...
  server = app.listen(port, () => {
    ...
    try { exec(`start ${url}`); } catch (e) { /* nÃ£o crÃ­tico */ }
  });
} else {
  // Em Vercel, nÃ£o iniciamos o listener. server fica undefined.
  server = undefined;
}

// Exporta o app (Express e uma funcao compativel com handler do Vercel).
// Anexamos propriedades uteis para testes locais (server) e para fechar o pool.
module.exports = app;
